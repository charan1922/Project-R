...
5. The script must:
   ...
   - **If `backtesting/projectr/` exists**: Use Project-R adapter for data fetching (`from projectr import dhan_client, signals, rfactor`). Uses Dhan V2 API (auth token shared with Next.js server). Benchmark via `dhan_client.fetch_benchmark()`. Signal cleaning via `projectr.signals.exrem()`. See vectorbt-expert `rules/projectr-data.md`.
   - **Otherwise**: Fetch data via `client.history()` from OpenAlgo
...
| R-Factor Filter | `r-factor-filter` | `assets/rfactor_filter/backtest.py` |

## Benchmark Rules

- Default: NIFTY 50 via OpenAlgo (`symbol="NIFTY", exchange="NSE_INDEX"`). If using Project-R adapter: `dhan_client.fetch_benchmark(start, end)`.

...
4. The script must:
   - **If `backtesting/projectr/` exists**: Use Project-R adapter (`from projectr import dhan_client, signals`). See vectorbt-expert `rules/projectr-data.md`.
   - **Otherwise**: Load `.env` from project root using `find_dotenv()` and fetch data via OpenAlgo `client.history()`
   ...
   - If `openalgo.ta` is not importable (standalone DuckDB or Project-R), use `projectr.signals` or inline `exrem()` fallback.

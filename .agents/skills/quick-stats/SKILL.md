...
1. Fetch data from Project-R adapter if `backtesting/projectr/` exists (`from projectr import dhan_client, signals`), otherwise from OpenAlgo (or DuckDB if user provides a DB path, or yfinance as fallback). See vectorbt-expert `rules/projectr-data.md`.

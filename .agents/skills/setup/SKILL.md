...
- **Indian Markets (Project-R / Dhan V2)** — uses `backtesting/projectr/` adapter, reads `.env.local`, no OpenAlgo needed. Auth token shared with Next.js server. Includes R-Factor signals.
...
**6b2. If the user selected Indian Markets (Project-R):**
- Skip OpenAlgo API key prompt — not needed
- Check that `.env.local` exists and has `DHAN_CLIENT_ID` set
- Check that `data/.dhan-token.json` exists (generated by running `pnpm dev`)
- If token file missing, inform user: "Run `pnpm dev` first to auto-generate the Dhan auth token, then re-run setup."
- Verify `backtesting/projectr/` exists. If not, inform user the adapter package needs to be installed.
- Write `.env` with a comment pointing to `.env.local` for Dhan credentials:
```
# Indian Markets (Project-R / Dhan V2)
# Dhan credentials are in .env.local (shared with Next.js server)
# No additional API keys needed for backtesting
```

**6c. If the user selected Indian Markets (OpenAlgo)**, ask for their OpenAlgo API key:

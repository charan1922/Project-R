...
- Data sources: OpenAlgo (Indian markets), **Project-R adapter** (Dhan V2 + SQLite bhavcopy + R-Factor signals), DuckDB (direct database), yfinance (US/Global), CCXT (Crypto), custom providers
...
- Signal cleaning: `openalgo.ta` for exrem, crossover, crossunder, flip. If `backtesting/projectr/` exists, `projectr.signals` provides standalone exrem/crossover/crossunder (no OpenAlgo dependency).
...
| [projectr-data](rules/projectr-data.md) | Project-R adapter: Dhan V2 API, bhavcopy SQLite, R-Factor signals, F&O universe |
...
| R-Factor Filter | `assets/rfactor_filter/backtest.py` | R-Factor institutional activity filter + EMA exit (Project-R) |

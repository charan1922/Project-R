...
export async function POST(request: Request) {
  try {
    ...
    if (!TOP_50_STOCKS.includes(upperSymbol)) {
      return NextResponse.json({ error: `Symbol ${upperSymbol} not found in NSE F&O top 50 stocks` }, { status: 400 });
    }
    ...
    const stdReturn =
      returns.length > 1 ? Math.sqrt(returns.reduce((s, r) => s + (r - meanReturn) ** 2, 0) / (returns.length - 1)) : 1;
...
export async function GET() {
  return NextResponse.json({
    ...
    regimes: REGIMES,
    description:
      'NSE F&O Top 50 stocks backtest API. POST with { symbol, days, rThreshold, volWeight, turnWeight, oiWeight, spreadWeight }',
  });
}

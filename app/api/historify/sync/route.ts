...
export async function POST(request: NextRequest) {
  try {
    ...
    const targets = isBatch
      ? payload.symbols.map((s: any) => ({
          symbol: s.symbol || s,
          exchange: s.exchange || payload.exchange || 'NSE',
          interval: payload.interval || 'Daily',
      ...
      try {
        ...
        if (fs.existsSync(parquetPath)) {
          ...
          const res = await c.run(
            `SELECT MAX(timestamp) as max_ts FROM read_parquet('${parquetPath}') WHERE interval = '${interval}'`,
          );
        ...
        if (rows > 0) {
          ...
          try {
            ...
            if (fs.existsSync(parquetPath)) {
              ...
              await conn.run(
                `COPY (SELECT * FROM read_json_auto('${tempJsonPath}') ORDER BY timestamp ASC) TO '${parquetPath}' (FORMAT PARQUET)`,
              );
            }
          } finally {

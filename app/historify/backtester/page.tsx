        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 space-y-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 mt-1">{STRATEGIES.find((s) => s.value === strategy)?.desc}</p>
            </div>
            ...
            {error && (
              <div className="bg-red-900/30 border border-red-700 rounded-lg p-3 text-red-300 text-xs">{error}</div>
            )}
          </div>
            ...
            {!result && !loading && (
              <div className="bg-slate-900/30 border border-slate-800 rounded-xl p-12 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-slate-600 max-w-md">
                  Select a strategy, enter a symbol, and click "Run Backtest" to test against historical data with
                  realistic Indian brokerage fees.
                </p>
              </div>
            )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    ...
                    },
                    {
                      ...
                      color:
                        result.profitFactor === null || result.profitFactor >= 1.5
                          ? 'text-green-400'
                          : 'text-yellow-400',
                      icon: BarChart2,
                    },
                    {
                      label: 'Strategy',
                      value: STRATEGIES.find((s) => s.value === result.strategy)?.label || result.strategy,
                    ...
                    <tbody>
                      {result.trades.slice(0, 50).map((t, i) => (
                        <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                          ...
                          <td
                            className={`py-1.5 pr-3 text-right font-medium ${t.pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}
                          >
                            {fmtCurrency(t.pnl)}
                          </td>

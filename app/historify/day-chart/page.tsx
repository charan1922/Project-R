...
const TRADEFINDER_MAP = new Map(_analytics.stockList.map((s) => [s.name.toUpperCase(), s]));
  ...
  const loadChart = useCallback(async () => {
    ...
    try {
      ...
      const wData = (data.indicators?.vwap || []).map((v: any, i: number) => ({
        time: sorted[i].timestamp as Time,
        value: v.value,
      }));
  ...
  return (
    <div className="min-h-screen bg-[#070d1a] text-slate-100 flex flex-col">
      ...
      <div className="px-6 pt-6 pb-4 border-b border-slate-800/70 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-slate-500">
            F&amp;O daily &amp; intraday ·{' '}
            {tradedOnly ? `${TRADED_FNO.length} Tradefinder stocks` : `${ALL_FNO.length} F&O stocks`} · NSE · Dhan V2
          </p>
        </div>
        ...
        {dayChange && (
          <div className="flex items-center gap-3 text-right shrink-0">
            ...
            <div>
              <div className="text-lg font-bold text-white font-mono">{ohlcv ? `₹${fmtPrice(ohlcv.close)}` : '—'}</div>
              <div className={`text-xs font-semibold ${dayChange.pct >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                {dayChange.pct >= 0 ? '+' : ''}
                {dayChange.pct.toFixed(2)}% &nbsp;({dayChange.abs >= 0 ? '+' : ''}
                {fmtPrice(dayChange.abs)})
              </div>
            </div>
          </div>
        )}
      ...
      <div className="px-6 py-3 border-b border-slate-800/50 flex flex-wrap items-center gap-3 bg-slate-900/30">
          ...
          <div className="relative w-44">
            ...
            {symbolOpen && (
              <div className="absolute z-50 top-full left-0 mt-1 w-full max-h-64 overflow-y-auto bg-slate-800 border border-slate-700 rounded-md shadow-xl">
                {filteredSymbols.length === 0 && <div className="px-3 py-2 text-xs text-slate-500">No matches</div>}
          ...
          {INTERVALS.map(({ value, label }) => (
            <button
              ...
              className={`px-2.5 py-1 text-xs font-bold rounded transition-all ${
                interval === value ? 'bg-rose-500 text-white shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              {label}
            </button>
          ...
          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-72 hidden group-hover:block z-50 pointer-events-none">
            <div className="bg-slate-900 border border-slate-700 rounded-lg p-3 shadow-xl text-[11px] text-slate-300 space-y-1.5">
              ...
              <div className="flex items-start gap-2">
                ...
                <span>
                  Go to <strong className="text-white">Watchlist</strong> → add symbols (e.g. ADANIGREEN)
                </span>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-rose-400 font-bold shrink-0">2.</span>
                <span>
                  Go to <strong className="text-white">Download</strong> → select symbol, pick interval &amp; date range
                  → click Download
                </span>
              </div>
              <div className="flex items-start gap-2">
                ...
                <span>
                  Come back here → select symbol &amp; date → <strong className="text-white">Load Chart</strong>
                </span>
              </div>
            </div>
          ...
          {[
            ...
            <button
              ...
              className={`px-2 py-1 text-[10px] font-bold rounded border transition-all ${
                btn.active ? btn.color : 'text-slate-500 border-slate-700/50 hover:bg-slate-800'
              }`}
            >
              {btn.label}
            </button>
        ...
        {tfStats && (
          <div className="ml-auto flex items-center gap-3 text-[11px] text-slate-400 bg-slate-900 border border-slate-800 rounded-lg px-3 py-1.5">
            ...
            <span className={tfStats.winRate >= 60 ? 'text-emerald-400' : 'text-red-400'}>
              {tfStats.winRate.toFixed(0)}% WR
            </span>
            <span className="text-slate-600">·</span>
      ...
      <div className="flex-1 flex flex-col px-6 py-4 gap-2 min-h-0">
        {/* Main candle chart */}
        <div
          className="relative w-full bg-[#0a0f1a] border border-slate-800/60 rounded-xl overflow-hidden"
          style={{ height: 440 }}
        >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0a0f1a]/95 gap-3">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl px-5 py-4 text-[11px] text-slate-400 space-y-2 max-w-xs text-left">
                ...
                <div className="flex items-start gap-2">
                  <span className="text-rose-400 font-bold shrink-0">1.</span>
                  <span>
                    <strong className="text-slate-300">Watchlist</strong> — ensure {symbol} is added
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-rose-400 font-bold shrink-0">2.</span>
                  <span>
                    <strong className="text-slate-300">Download</strong> — select {symbol}, interval{' '}
                    <code className="bg-slate-800 px-1 rounded">{interval}</code>, date range including {date}
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-rose-400 font-bold shrink-0">3.</span>
                  <span>Come back here and load again</span>
                </div>
              </div>
              <a
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none gap-3">
              ...
              <div className="mt-2 bg-slate-900/80 border border-slate-800 rounded-xl px-5 py-4 text-[11px] text-slate-400 space-y-2 max-w-xs text-left">
                ...
                <div className="flex items-start gap-2">
                  <span className="text-rose-400 font-bold shrink-0">1.</span>
                  <span>
                    <strong className="text-slate-300">Watchlist</strong> — add your F&amp;O symbols first
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-rose-400 font-bold shrink-0">2.</span>
                  <span>
                    <strong className="text-slate-300">Download</strong> — select symbol, interval &amp; date range →
                    Download Data (stores to local DB)
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-rose-400 font-bold shrink-0">3.</span>
                  <span>
                    Return here, select symbol &amp; date → <strong className="text-slate-300">Load Chart</strong>
                  </span>
                </div>
              </div>
            </div>
        ...
        {/* Volume chart */}
        <div
          className="relative w-full bg-[#0a0f1a] border border-slate-800/40 rounded-lg overflow-hidden"
          style={{ height: 90 }}
        >
          <div className="absolute top-1.5 left-3 z-10 text-[9px] text-slate-600 font-semibold uppercase tracking-widest">
            Volume
          </div>
      ...
      <div className="px-6 py-2 border-t border-slate-800/50 flex items-center gap-4 text-[10px] text-slate-600 bg-slate-900/20">
        <span className="flex items-center gap-1">
          <AlertCircle className="w-3 h-3" /> Data via Dhan V2 · better-sqlite3
        </span>
        <span>·</span>
        <span>
          F&amp;O Universe: {ALL_FNO.length} stocks · TF Traded: {TRADED_FNO.length}
        </span>
      </div>
    </div>
  );

...
export default function DownloadPage() {
  ...
  const toggleAll = () =>
    setCheckedSymbols(checkedSymbols.size === watchlist.length ? new Set() : new Set(watchlist.map((w) => w.symbol)));
  ...
  const startDownload = async () => {
    ...
    for (const job of newJobs) {
      setJobs((prev) =>
        prev.map((j) => (j.id === job.id ? { ...j, status: 'downloading', eta: 'Syncing…', progress: 50 } : j)),
      );
      try {
        ...
        setJobs((prev) =>
          prev.map((j) =>
            j.id === job.id ? { ...j, progress: 100, status: ok ? 'complete' : 'failed', rows, eta: '—' } : j,
          ),
        );
      } catch {
  ...
  const inputCls =
    'w-full px-3 py-2 text-sm bg-slate-800 border border-slate-700 rounded-lg text-slate-200 focus:outline-none focus:border-sky-500';
      ...
      <div className="px-6 pt-8 pb-6 border-b border-slate-800/60">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <button
            ...
            disabled={checkedSymbols.size === 0 || loading}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              ...
              {watchlist.length === 0 ? (
                <div className="flex items-center gap-2 px-3 py-3 bg-amber-500/10 border border-amber-500/20 rounded-lg text-sm text-amber-400">
                  ...
                  <span>
                    Watchlist empty.{' '}
                    <a href="/historify/watchlist" className="underline">
                      Add symbols
                    </a>{' '}
                    first.
                  </span>
                ...
                <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-2 max-h-48 overflow-y-auto space-y-0.5">
                  {watchlist.map((w) => (
                    <label
                      key={w.symbol}
                      className="flex items-center gap-3 cursor-pointer hover:bg-slate-700/30 rounded-md px-2 py-1.5"
                    >
                      <input
                        type="checkbox"
                        checked={checkedSymbols.has(w.symbol)}
                        onChange={() =>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1.5 block font-medium">
                  Data Interval
                </label>
                ...
                <div key={d.id} className="p-4 hover:bg-slate-800/30">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-xs font-medium ${d.status === 'complete' ? 'text-emerald-400' : d.status === 'downloading' ? 'text-sky-400' : d.status === 'failed' ? 'text-red-400' : 'text-slate-500'}`}
                      >
                        {d.status === 'complete'
                          ? 'Done'
                          : d.status === 'downloading'
                      ...
                      {d.status === 'failed' && (
                        <button
                          className="p-1 rounded hover:bg-slate-700 text-slate-500 hover:text-sky-400"
                          title="Retry"
                        >
                      ...
                      {d.status === 'downloading' && (
                        <button
                          className="p-1 rounded hover:bg-slate-700 text-slate-500 hover:text-amber-400"
                          title="Pause"
                        >
                  ...
                  <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${d.status === 'complete' ? 'bg-emerald-500' : d.status === 'failed' ? 'bg-red-500' : 'bg-sky-500'}`}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden">
          ...
          {history.length === 0 ? (
            <div className="p-8 text-center">
              <p className="text-sm text-slate-600">No download history yet.</p>
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800 text-slate-500 text-xs uppercase">
                  ...
                  <th className="p-3 text-left">Interval</th>

          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 text-sm bg-slate-800 border border-slate-700 rounded-lg text-slate-200 focus:outline-none focus:border-violet-500"
              >
                {INTERVALS.map((i) => (
            ...
            <div className="flex flex-wrap gap-2">
              {PRESETS.map((p) => (
                <button
                  key={p}
                  onClick={() => setPreset(p)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${preset === p ? 'bg-violet-500/20 text-violet-400 border-violet-500/30' : 'bg-slate-800 text-slate-400 border-slate-700 hover:border-slate-600'}`}
                >
                  {p}
                </button>
              ))}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-semibold text-white">
                        {item.symbols.length} symbol{item.symbols.length !== 1 ? 's' : ''}
                      </span>
                    ...
                    <div className="flex items-center gap-2">
                      {item.status === 'complete' ? (
                        ...
                        <>
                          ...
                          <span className="text-xs text-sky-400">Exporting…</span>
                        </>
                      ) : (
                        <>

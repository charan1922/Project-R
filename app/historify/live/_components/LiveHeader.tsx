        ...
        <div>
          ...
          <p className="text-xs font-mono text-slate-500 flex items-center gap-2">
            <span className="flex h-2 w-2 relative">
              <span
                className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${connectionStatus === 'connected' ? 'bg-emerald-400' : 'bg-rose-400'}`}
              ></span>
              <span
                className={`relative inline-flex rounded-full h-2 w-2 ${connectionStatus === 'connected' ? 'bg-emerald-500' : 'bg-rose-500'}`}
              ></span>
            </span>
      ...
      {activeSymbol && latestTick && (
        <div className="flex items-center gap-6 text-sm font-mono border border-white/10 rounded-lg px-4 py-2 bg-slate-900/80">
          <div className="flex flex-col">
            ...
            <span
              className={`font-bold text-lg ${latestTick.close >= latestTick.open ? 'text-emerald-400' : 'text-rose-400'}`}
            >

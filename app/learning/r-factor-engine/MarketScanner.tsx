      ...
      <CardHeader className="flex flex-col space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-4 p-2 rounded-lg bg-slate-950 border border-slate-800">
          ...
          <div className="flex items-center space-x-2">
            <Checkbox id="blast-only" checked={blastOnly} onCheckedChange={(checked) => setBlastOnly(!!checked)} />
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-xs font-mono font-bold ${sig.compositeRFactor > 0 ? 'text-emerald-400' : 'text-red-400'}`}
                    >
                  ...
                  <p className="text-[10px] text-slate-500 font-mono mt-1">
                    Vol:{' '}
                    <span className={sig.zScores.volume > 2 ? 'text-emerald-400' : ''}>
                      {sig.zScores.volume.toFixed(2)}σ
                    </span>{' '}
                    • Spr:{' '}
                    <span className={sig.zScores.spread > 1.5 ? 'text-amber-400' : ''}>
                      {sig.zScores.spread.toFixed(2)}σ
                    </span>
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {sig.isBlastTrade && (
                    <Badge className="bg-emerald-500/20 text-emerald-400 text-[10px] px-1.5 py-0 border border-emerald-500/30 animate-pulse">
                      BLAST
                    </Badge>
                  )}
                </div>

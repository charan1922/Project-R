...
const GRID =
  'grid grid-cols-[1.5fr_52px_78px_78px_78px_84px_84px_78px_84px_84px_90px_60px_70px] gap-2 px-4 min-w-[1180px]';
            ...
            <div key={s.symbol} className="border-b border-slate-800/50">
              {s.bhav && (
                <div className={`${GRID} py-1.5 text-sm hover:bg-slate-800/30`}>
                  ...
                  <span className="text-slate-400 font-mono">{fmtK(s.bhav.optVolume)}</span>
                  <span className="text-slate-400 font-mono">{fmtK(s.bhav.optOi)}</span>
                  <span className="text-slate-400 font-mono">{fmtMoney(s.bhav.optTurnover)}</span>
              ...
              {hasDhan && s.dhan && s.bhav && (
                <div className={`${GRID} py-1.5 text-sm hover:bg-slate-800/30 bg-slate-800/10`}>
                  ...
                  <span className={`font-mono ${diffColor(s.bhav.optVolume, s.dhan.optVolume) || 'text-slate-400'}`}>
                    {fmtK(s.dhan.optVolume)}
                  </span>
                  <span className={`font-mono ${diffColor(s.bhav.optOi, s.dhan.optOi) || 'text-slate-400'}`}>
                    {fmtK(s.dhan.optOi)}
                  </span>
                  <span
                    className={`font-mono ${diffColor(s.bhav.optTurnover, s.dhan.optTurnover) || 'text-slate-400'}`}
                  >
                    {fmtMoney(s.dhan.optTurnover)}
                  </span>
              ...
              {!s.bhav && s.dhan && (
                <div className={`${GRID} py-1.5 text-sm hover:bg-slate-800/30`}>
                  ...
                  <OiChg v={s.dhan.futOiChange} />
                  <span className="text-slate-400 font-mono">{fmtK(s.dhan.optVolume)}</span>
                  <span className="text-slate-400 font-mono">{fmtK(s.dhan.optOi)}</span>
                  <span className="text-slate-400 font-mono">{fmtMoney(s.dhan.optTurnover)}</span>
                  <span className="text-slate-400 font-mono">{pcr(s.dhan.ceVolume, s.dhan.peVolume)}</span>
                  <span className="text-violet-400 font-mono font-bold">{s.dhan.rFactor.toFixed(2)}</span>
                </div>
              )}

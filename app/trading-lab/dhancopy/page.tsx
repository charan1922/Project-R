...
import { BarChart2, ChevronDown, Loader2, RefreshCw, Search } from 'lucide-react';
...
export default function DhancopyPage() {
  ...
  const handleCompute = async () => {
    if (!computeDate || computing) return;
    setComputing(true);
    ...
    try {
      ...
      if (d.success) {
        ...
        setComputeResult(`Error: ${d.error}`);
      }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="px-2 py-1.5 rounded-lg bg-slate-900 border border-slate-700 text-sm text-slate-300 focus:outline-none focus:border-violet-500"
          />
          <button
            type="button"
            onClick={handleCompute}
            disabled={computing}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-violet-500/20 hover:bg-violet-500/30 text-violet-400 border border-violet-500/30 text-sm font-medium disabled:opacity-50"
          >
            {computing ? <Loader2 className="w-3 h-3 animate-spin" /> : null}
            {computing ? 'Computing...' : 'Fetch from Dhan'}
          </button>
          <button
            type="button"
      ...
      {computeResult && (
        <div
          className={`mb-4 px-4 py-3 rounded-lg text-sm ${computeResult.startsWith('Error') ? 'bg-red-500/10 border border-red-500/30 text-red-400' : 'bg-emerald-500/10 border border-emerald-500/30 text-emerald-400'}`}
        >
          {computeResult}
        </div>
      )}
        ...
        {!loading && rows.length === 0 && (
          <div className="px-5 py-16 text-center">
            ...
            <p className="text-sm text-slate-500">
              {total === 0
                ? 'No Dhan daily data cached yet. Go to Bhav vs Dhan page to compute.'

...
function getReason(stats: any) {
  if (stats.tier === 'A+')
    return `Elite: ${stats.winRate.toFixed(0)}% win rate across ${stats.count} trades with strong avg return.`;
...
export default function FnOUniversePage() {
  ...
  // Full universe: 206 official F&O stocks + removed-from-F&O stocks TF traded (with label)
  const universe = useMemo(() => {
    const uniqueFnoStocks = Array.from(new Set(fnoData.stocks.map((s) => s.toUpperCase())));
    const fnoSet = new Set(uniqueFnoStocks);

    // Official F&O stocks
    const all = uniqueFnoStocks.map((symbol) => {
      const stats = tradedMap.get(symbol) ?? null;
      return { symbol, symbolUpper: symbol, stats, traded: stats !== null, tag: '' as '' | 'removed' | 'index' };
    });

    // TF-traded stocks NOT in official F&O list — show with label
    analytics.stockList.forEach((s) => {
      const symUpper = s.name.toUpperCase();
      if (fnoSet.has(symUpper)) return; // Already in official list
      if (!symUpper || symUpper.includes('+') || symUpper === 'NULL') return; // Skip combos/nulls
      const isIndex =
        symUpper === 'NIFTY' || symUpper === 'BANKNIFTY' || symUpper === 'FINNIFTY' || symUpper === 'MIDCPNIFTY';
      if (isIndex) return; // Skip index trades — not stock F&O
      all.push({ symbol: s.name, symbolUpper: symUpper, stats: s, traded: true, tag: 'removed' });
    });
    return all;
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="px-6 py-5 max-w-7xl mx-auto">
          ...
          {(
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${
                tab === t.key ? 'bg-slate-700 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              {t.label}
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {filtered.map(({ symbol, stats, traded: isTr, tag }) => {
            if (!isTr) {
              return (
                <div
                  key={symbol}
                  className="bg-slate-900 border border-slate-800 rounded-lg p-3 flex items-center gap-3 hover:border-slate-700 transition-colors"
                >
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 flex-1 min-w-0">
                      ...
                      {tag === 'removed' && (
                        <span className="px-1 py-0.5 rounded bg-red-500/10 text-red-400 border border-red-500/20 text-[9px] font-bold shrink-0 leading-none">
                          REMOVED FROM F&O
                        </span>
                      )}
                    </div>
                  ...
                  <span
                    className={`px-1.5 py-0.5 rounded text-xs font-bold ${cfg.bg} ${cfg.text} border ${cfg.border} shrink-0`}
                  >
                    {stats.tier}
                  </span>
                </div>
                    ...
                    <div
                      className={`h-1 rounded-full ${
                        stats.compositeScore >= 80
                          ? 'bg-emerald-500'
                          : stats.compositeScore >= 65
                            ? 'bg-sky-500'
                            : stats.compositeScore >= 50
                              ? 'bg-indigo-500'
                              : stats.compositeScore >= 35
                                ? 'bg-amber-500'
                                : 'bg-red-500'
                      }`}
                ...
                <div className="grid grid-cols-3 gap-1 text-xs">
                  ...
                  <div>
                    ...
                    <div
                      className={`font-semibold ${stats.winRate >= 70 ? 'text-emerald-400' : stats.winRate >= 50 ? 'text-amber-400' : 'text-red-400'}`}
                    >
                      {pct(stats.winRate)}
                    </div>
                  </div>
          ...
          {filtered.length === 0 && (
            <div className="col-span-full py-16 text-center text-slate-500">No stocks match the current filters.</div>
          )}
        </div>
      </div>
    </div>

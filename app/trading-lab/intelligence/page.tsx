...
export default function IntelligenceDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const avgRFactor =
      signals.length > 0 ? signals.reduce((acc, s) => acc + s.compositeRFactor, 0) / signals.length : 0;
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-slate-400">7-Factor Institutional Activity Detection via NSE F&O Bhavcopy</p>
        </div>
        <div className="flex items-center gap-3">
          <Button onClick={fetchSignals} disabled={loading} className="bg-sky-600 hover:bg-sky-500">
      ...
      {/* Results Grid */}
      {error && (
        <div className="p-6 text-center rounded-lg border border-red-500/30 bg-red-500/10 text-red-400">{error}</div>
      )}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSignals.map((signal) => (
            <Card
              key={signal.symbol}
              className="bg-slate-900 border-slate-800 hover:border-sky-500/30 transition-all group overflow-hidden"
            >
              <div
                className={`h-1 w-full ${
                  signal.compositeRFactor > 2
                    ? 'bg-emerald-500'
                    : signal.compositeRFactor > 1
                      ? 'bg-sky-500'
                      : 'bg-slate-700'
                }`}
              />
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mt-1">
                      <Badge
                        variant="outline"
                        className={`text-[10px] ${
                          signal.regime === 'Cheetah'
                            ? 'border-amber-500/50 text-amber-500'
                            : signal.regime === 'Elephant'
                              ? 'border-sky-500/50 text-sky-500'
                              : signal.regime === 'Hybrid'
              ...
              <CardContent>
                ...
                <div className="mt-4 pt-3 border-t border-slate-800/50 flex justify-between items-center">
                  <span className="text-[10px] text-slate-600">{new Date(signal.timestamp).toLocaleTimeString()}</span>
        ...
        <CardContent className="p-4 flex gap-4 items-start">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">
              Compares today&apos;s F&amp;O activity vs 20-day average using a hybrid ratio+Z-score model across 7
              factors: Spread Ratio (30%), Futures Turnover (20%), Put-Call Ratio (15%), OI Change (12%), Trade Size
              (10%), Futures Volume (8%), Options Volume (5%). Validated against 80 F&amp;O stocks.

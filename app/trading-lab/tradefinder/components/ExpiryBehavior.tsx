...
import { ProgressRow, Section } from './ui';
...
export function ExpiryBehavior({ data }: Props) {
  ...
  return (
    <Section title="8. Strike Price & Expiry Behavior" icon={Target} color="text-violet-400">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
        <div>
          <p className="text-xs text-slate-500 uppercase font-semibold mb-3">Moneyness (where spot_price available)</p>
          ...
          <div className="space-y-3">
            <div className="bg-sky-500/10 border border-sky-500/20 rounded-lg p-4">
              ...
              <div className="text-xs text-slate-400 mt-1">
                Total P&L:{' '}
                <span className={monthlyPnl >= 0 ? 'text-emerald-400' : 'text-red-400'}>{fmt(monthlyPnl)}</span>
              </div>
            </div>
            <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-4">
              ...
              <div className="text-xs text-slate-400 mt-1">
                Total P&L:{' '}
                <span className={weeklyPnl >= 0 ? 'text-emerald-400' : 'text-red-400'}>{fmt(weeklyPnl)}</span>
              </div>
            </div>
          </div>
        </div>

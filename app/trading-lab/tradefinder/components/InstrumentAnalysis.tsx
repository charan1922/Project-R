...
import type { AnalyticsResult, RawTrade } from '../types';
    ...
    <div className={`${bg} ${border} border rounded-lg p-3`}>
      ...
      <div className="grid grid-cols-3 gap-2 text-xs">
        ...
        <div>
          ...
          <br />
          <span className={pnl >= 0 ? 'text-emerald-400 font-semibold' : 'text-red-400 font-semibold'}>{fmt(pnl)}</span>
        </div>
        <div>
          <span className="text-slate-500">Win Rate</span>
          <br />
          <span className="text-white font-semibold">{trades.length ? pct((wins / trades.length) * 100) : '—'}</span>
        </div>
      </div>
      <div className="mt-1 text-xs text-slate-400">
        Avg P&L:{' '}
        <span className={pnl >= 0 ? 'text-emerald-400' : 'text-red-400'}>{fmt(pnl / (trades.length || 1))}</span>
      </div>
    </div>
  );
}
...
export function InstrumentAnalysis({ data: { ceT, peT } }: Props) {
  ...
  return (
    <Section title="6. Instrument Analysis (CE vs PE)" icon={Target} color="text-pink-400">

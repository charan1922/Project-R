...
export function OverallPerformance({ data, sortedStocksByCount }: Props) {
  const { actual, noTrade, wins, losses, totalPnl, grossProfit, grossLoss, months } = data;
  ...
  return (
    <Section title="1. Overall Performance" icon={TrendingUp}>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-1">
        ...
        <StatCard
          label="Win Rate"
          value={pct((wins.length / actual.length) * 100)}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
        ...
        <StatCard
          ...
          value={fmt(bestDay?.total_pnl ?? 0)}
          sub={bestDay?.trade_date}
          positive={true}
        />
        <StatCard
          label="Worst Single Day"
          value={fmt(worstDay?.total_pnl ?? 0)}
          sub={worstDay?.trade_date}
          positive={false}
        />
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
        <StatCard
          label="Overall ROI"
          value={fmtROI(data.capitalAnalysis.overallROI)}
          positive={data.capitalAnalysis.overallROI >= 0}
        />
        <StatCard
          label="Avg ROI / Trade"
          value={fmtROI(data.capitalAnalysis.avgROIPerTrade)}
          positive={data.capitalAnalysis.avgROIPerTrade >= 0}
        />
        <StatCard
          label="Human Verified"
          value={`${data.humanReviewedCount} of ${actual.length}`}
          sub="Trades with broker verification"
        />
        <StatCard label="Avg Capital / Trade" value={fmt(data.capitalAnalysis.avgCapitalPerTrade)} />
      </div>
    </Section>
  );
}

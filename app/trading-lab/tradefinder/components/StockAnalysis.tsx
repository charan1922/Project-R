...
export function StockAnalysis({ stockList }: Props) {
  ...
  return (
    <Section title="7. Stock-wise Analysis" icon={Award} color="text-amber-400">
      <div className="space-y-5 mt-2">
        <div>
          ...
          <Table
            ...
            rows={byCount.slice(0, 10).map((s) => [
              ...
              <span key="w" className="text-emerald-400">
                {s.wins}
              </span>,
              <span key="l" className="text-red-400">
                {s.losses}
              </span>,
              pct(s.winRate),
            ])}
        ...
        <div>
          ...
          <Table
            headers={['Stock', 'Total P&L', 'Avg ROI', 'Trades', 'Win %']}
            rows={byPnl.map((s) => [
              <StockName key={s.name} name={s.name} />,
              <PnLCell key="p" v={s.pnl} />,
              <span key="r" className={`font-semibold ${s.avgROI >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                {fmtROI(s.avgROI)}
              </span>,
              s.count,
              pct(s.winRate),
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {perfect.length === 0 ? (
              ...
              <Table
                ...
                rows={perfect.slice(0, 8).map((s) => [
                  <span key={s.name} className="text-emerald-400 font-mono text-xs">
                    {s.name}
                  </span>,
                  s.count,
                  <PnLCell key="p" v={s.pnl} />,
          ...
          <div>
            ...
            <Table
              ...
              rows={bestSingle.map((s) => [<StockName key={s.name} name={s.name} />, <PnLCell key="p" v={s.maxPnl} />])}
            />
          </div>
        </div>
      </div>

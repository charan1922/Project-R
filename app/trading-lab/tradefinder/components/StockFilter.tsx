...
import { PnLCell, Section } from './ui';
...
const TIER_CONFIG: Record<string, { label: string; bg: string; text: string; border: string; desc: string }> = {
  'A+': {
    ...
    desc: 'Score ≥80: Elite — consistently profitable, high confidence',
  },
  A: {
    label: 'A',
    bg: 'bg-sky-500/15',
    ...
    desc: 'Score 65–79: Strong — good win rate and average return',
  },
  B: {
    label: 'B',
    bg: 'bg-indigo-500/15',
    ...
    desc: 'Score 50–64: Decent — tradeable, needs bigger sample',
  },
  C: {
    label: 'C',
    bg: 'bg-amber-500/15',
    ...
    desc: 'Score 35–49: Marginal — low win rate or small sample',
  },
  D: {
    label: 'D',
    bg: 'bg-red-500/15',
    ...
    desc: 'Score <35: Avoid — consistent losses or very low win rate',
  },
};
...
export function StockFilter({ stockList }: Props) {
  ...
  const [sortBy, setSortBy] = useState<'score' | 'pnl' | 'winRate' | 'count' | 'roi'>('score');
  ...
  return (
    <Section title="0. F&O Stock Filter — Tradefinder Scored Universe" icon={Filter} color="text-violet-400">
      {/* Purpose banner */}
      <div className="mt-2 mb-4 p-4 bg-violet-500/10 border border-violet-500/20 rounded-xl">
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-slate-300">
            <span className="text-violet-300 font-semibold">Mission:</span> Filter the F&O universe (240+ stocks) using
            Tradefinder_02's real verified trade history. Each stock gets a{' '}
            <span className="text-white font-semibold">composite score 0–100</span> based on win rate (40%), average P&L
            (30%), sample size confidence (20%), and loss safety (10%). Focus trading on{' '}
            <span className="text-emerald-400 font-semibold">A+ and A</span> tier stocks.
          </div>
        </div>
      </div>
      ...
      <p className="text-xs text-slate-500 mb-3">
        ...
        {tierFilter !== 'All' && (
          <>
            {' '}
            in tier <TierBadge tier={tierFilter as StockStats['tier']} />
          </>
        )}
        {search && (
          <>
            {' '}
            matching "<span className="text-sky-400">{search}</span>"
          </>
        )}
          ...
          <thead className="sticky top-0 bg-slate-900/95 backdrop-blur-sm z-10">
            <tr className="border-b border-slate-700/60">
              {[
                ...
                <th
                  key={h}
                  className="text-left py-2.5 px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {filtered.map((s, i) => (
              <tr key={s.name} className="border-b border-slate-800/40 hover:bg-slate-800/40 transition-colors">
                ...
                <td className="py-2 px-3">
                  <span
                    className={`font-semibold text-xs ${s.avgROI >= 50 ? 'text-emerald-400' : s.avgROI >= 0 ? 'text-sky-400' : 'text-red-400'}`}
                  >
                    {fmtROI(s.avgROI)}
                  </span>
                </td>

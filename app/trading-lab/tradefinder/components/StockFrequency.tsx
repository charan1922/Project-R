...
export function StockFrequency({ stockList }: Props) {
  ...
  return (
    <Section title="13. Stock Trade Frequency — Full Ranking" icon={BarChart2} color="text-sky-400">
      ...
      <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {sorted.map((s, i) => (
              <tr key={s.name} className="border-b border-slate-800/40 hover:bg-slate-800/30 transition-colors">
                ...
                <td className="py-2 px-3">
                  <span
                    className={`font-semibold ${
                      s.winRate >= 80 ? 'text-emerald-400' : s.winRate >= 50 ? 'text-amber-400' : 'text-red-400'
                    }`}
                  >
                    {pct(s.winRate)}
                  </span>

...
/** Parse "10:23:22 AM" or "03:48 PM" → hour in 24h format. Returns null on bad input */
function parse12hToHour(t: string): number | null {
...
/** Parse "10:23:22 AM" → minutes since midnight. Handles both HH:MM:SS and HH:MM formats */
...
/** Compute holding duration in minutes between entry and exit time strings */
export function holdingDurationMinutes(entry: string, exit: string): number | null {
  const e = parseTimeToMinutes(entry);
  const x = parseTimeToMinutes(exit);
  if (e === null || x === null) return null;
  return x - e;
}

/** Format duration in minutes → "2h 18m" or "42m" */
...
/** Format ROI with sign: "+72.1%" or "-12.3%" */
export function fmtROI(roi: number): string {
  const sign = roi >= 0 ? '+' : '';
  return `${sign}${roi.toFixed(1)}%`;
}

/** Time bucket using entry_time (HH:MM:SS AM/PM) or sensibull_log_time (HH:MM AM/PM) */
export function timeBucketAny(t: string | null): string {
  if (!t) return 'Unknown';
  const hour = parse12hToHour(t);

...
import { KillSwitchAction, KillSwitchResponse, PnlExitRequest, SetIpRequest, IpResponse } from '../types';
...
export class TraderControlApi {
  ...
  async activateKillSwitch(action: KillSwitchAction): Promise<KillSwitchResponse> {
    const resp = await this.http.post<KillSwitchResponse>(`/killswitch?killSwitchStatus=${action}`, {});
    return resp.data;
  }

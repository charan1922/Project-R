...
/** Format a timestamp (seconds) to IST YYYY-MM-DD */
function formatDateIST(epochSec: number): string {
...
async function fetchFromParquet(symbol: string, startDate: string, endDate: string): Promise<OHLCVRow[]> {
  try {
...
async function fetchFromDhanAPI(
  ...
  try {
    ...
    const { getDhanAccessToken, hasDhanAuth } = await import('@/lib/dhan/auth');
    if (!hasDhanAuth()) return [];
    const clientId = appEnv.DHAN_CLIENT_ID!;
    const accessToken = await getDhanAccessToken();

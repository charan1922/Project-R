...
export function computeSingleRRG(
  ...
  const rawRS = sectorCloses.map((s, i) => (benchmarkCloses[i] !== 0 ? (s / benchmarkCloses[i]) * 100 : NaN));
  ...
  const rsMomentumRaw: number[] = rsRatio.map((val, i) => (i === 0 ? NaN : val - rsRatio[i - 1]));
 ...
 */
export async function computeFullRRG(benchmarkSymbol = 'NIFTY', tailLength = 8): Promise<RRGResult> {

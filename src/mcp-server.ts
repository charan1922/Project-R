...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (request.params.name) {
        ...
        content: [
          {
            type: 'text',
            text:
              `Found ${filtered.length} trades for ${symbol}:\n\n` +
              filtered
                .map(
                  (t) => `${t.Date} | ${t.Option_Type} | Qty: ${t.Qty} | P&L: ₹${parseFloat(t.P_L).toLocaleString()}`,
                )
                .join('\n'),
          },
        ],
        ...
        content: [
          {
            type: 'text',
            text:
              `Found ${filtered.length} trades on ${date}:\n\n` +
              filtered
                .map(
                  (t) => `${t.Symbol} ${t.Option_Type} | Qty: ${t.Qty} | P&L: ₹${parseFloat(t.P_L).toLocaleString()}`,
                )
                .join('\n'),
          },
        ],
        ...
        content: [
          {
            type: 'text',
            text:
              `Found ${filtered.length} ${type} trades:\n\n` +
              filtered
                .slice(0, 20)
                .map(
                  (t) =>
    ...
    case 'get_daily_pnl': {
      ...
      const sorted = Array.from(dailyMap.entries()).sort((a, b) => new Date(b[0]).getTime() - new Date(a[0]).getTime());
      return {
        content: [
          {
            type: 'text',
            text:
              `Daily P&L Summary (last 30 days):\n\n` +
              sorted
                .slice(0, 30)
                .map(([date, data]) => `${date} | Trades: ${data.count} | Daily P&L: ₹${data.pnl.toLocaleString()}`)
                .join('\n'),
          },
        ],
      };
    ...
    case 'get_top_profitable_trades': {
      ...
      const sorted = [...trades].sort((a, b) => parseFloat(b.P_L) - parseFloat(a.P_L)).slice(0, limit);
      return {
        content: [
          {
            type: 'text',
            text:
              `Top ${limit} Most Profitable Trades:\n\n` +
              sorted
                .map(
                  (t, i) =>
    ...
    case 'get_top_losing_trades': {
      const limit = (request.params.arguments as any).limit || 10;
      const sorted = [...trades].sort((a, b) => parseFloat(a.P_L) - parseFloat(b.P_L)).slice(0, limit);
      return {
        content: [
          {
            type: 'text',
            text:
              `Top ${limit} Biggest Losing Trades:\n\n` +
              sorted
                .map(
                  (t, i) =>
        ...
        content: [
          {
            ...
            text:
              ...
              symbolTrades
                .slice(-5)
                .map((t) => `  ${t.Date} | ${t.Option_Type} | P&L: ₹${parseFloat(t.P_L).toLocaleString()}`)
                .join('\n'),
          },
        ],
      };
        ...
        content: [
          {
            type: 'text',
            text:
              `Found ${filtered.length} trades matching "${query}":\n\n` +
              filtered
                .slice(0, 20)
                .map(
                  (t) =>
          ...
          content: [
            {
              type: 'text',
              text:
                `Market Scan Results (Limit: ${limit}):\n\n` +
